...
module.exports.createTransaction = async (req, res) => {
  ...
  return res.status(400).json({ message: 'Transaction failed', data, error });
};
...
module.exports.generateStatementPDF = async (req, res) => {
  ...
  pdf.create(statementTemplate(req.body, req.user), {
    format: 'A4', width: '8.26in', height: '11.69in', orientation: 'portrait',
  }).toFile(`${user.email}.pdf`, (err) => {
    if (err) {
...
module.exports.generateNodTransferPDF = async (req, res) => {
  ...
  pdf.create(txnMap[txn.receive_method](txn, user), {
    format: 'A4', width: '8.26in', height: '11.69in', orientation: 'portrait',
  }).toFile(`${txn.reference}.pdf`, (err) => {
    if (err) {
      // return Promise.reject(err);
